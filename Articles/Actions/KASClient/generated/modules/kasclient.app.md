...
| callback | `function` |  callback with below parameters:<br><br>\* @param {string} response response body returned<br><br> This could have two possible config: If request was a success it returns jsonstring with following keys:<br><br> a.) "HttpResponseCode" : The response code of request.<br><br> b.) "HttpResponseHeader": The response HTTP headers<br><br> c.) "HttpResponseBody": The response body returned for request.<br><br> If there was a Network error then it returns:<br><br> a.) "HttpErrorCode": The error code<br><br> b.) "HttpErrorMessage": The error message eg. Malformed URL, Cannot connect to host etc.<br><br>\* @param {string} error error if any : This includes the standard error code defined in KASClient documentation. |
